...
import cn.edu.pku.EOSCN.crawler.ApacheMboxCrawler;
import cn.edu.pku.EOSCN.crawler.mailcrawlerthread.Configurable;
...
* @date 2013-6-3 上午10:41:26
*/

public class CrawlerController extends Configurable{
	protected static final Logger logger = Logger.getLogger(CrawlerController.class.getName());
	
	protected HtmlDocCrawler fatherThread;
	...
	public CrawlerController(HtmlDocCrawler fatherThread,CrawlerConfig config, URLManager urlManager,Project project) {
		super(config);
	...
	public void start(MainHtmlDocCrawler c, int numOfCrawlers, boolean isBlocking) {
		try{
			finished = false;
			final List<Thread> threads = new ArrayList<Thread>();
			final List<MainHtmlDocCrawler> crawlers = new ArrayList<MainHtmlDocCrawler>();

			for (int i = 1; i <= numOfCrawlers; i++) {
				MainHtmlDocCrawler crawler = new MainHtmlDocCrawler();
						...
						synchronized (waitingLock) {
							...
							while (true) {
								...
								if (!someoneIsWorking) {
									...
									for (int i = 0; i < threads.size(); i++) {
										Thread thread = threads.get(i);
										if (thread.isAlive() && !crawlers.get(i).isWaitingForNewURL()) {
											someoneIsWorking = true;
										}
									}
									if (!someoneIsWorking) {
										if (!shutdown) {
											if(!(urlManager.getWaitingQueue().size() == 0)) continue;
										}
